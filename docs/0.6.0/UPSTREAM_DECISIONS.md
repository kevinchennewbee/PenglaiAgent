...
| Electron (loong64 / UOS 20) | none | Official 43.6.0 has no loong64 zip. darkyzhou 43.4.1 requires glibc ≥ 2.38 **new-world** — incompatible with this host. Loongson ftp still ships `v22.3.27` (electerm old-world legacy uses it) through `v31.7.7`. | **Mac/Windows stay 43.6.0.** UOS 20 cannot use that zip. Demonstrated old-world train is Loongson **Electron 22.3.27**; 31.7.7 needs ELF world proof. Disclose Chromium generation vs 150. Never `--no-sandbox`. See `docs/0.6.0/UOS.md`. |
