...
6. **Still required:** native installed onboarding and plugin enable/disable/restart, real 0.6.5-to-0.6.6 migration, third-party manager consent, Weixin/Feishu live-account delivery where credentials are available, Memory-real on a clean source SHA, three target artifacts, and immutable public-byte readback. UOS package/ABI evidence is distinct from UOS native-machine evidence. Public README/site must remain on 0.6.5 until readback.
