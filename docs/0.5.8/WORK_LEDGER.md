...
| P058-047 | Canonical source replay and content-addressed lock | Source/package/CI | IN_PROGRESS | two canonical full builds and the promoted 251-package closure are recursively byte-identical locally; same-version lock reseal preserves unrelated dependencies; native CI replay remains |
| P058-048 | Distribution-owned document title | Source/UI/native | IN_PROGRESS | official DSH packages retain their upstream title while the Penglai preload owns the document title; unit and desktop E2E pass; installed plugin walk remains |
