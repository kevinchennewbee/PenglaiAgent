...
> runtime pin is official DSH `0.1.6-alpha.2`; see
> `release-contract.json` and `docs/0.6.5/UPSTREAM_DECISIONS.md`. Older source
...
- CPU graph runtime：`onnxruntime-node@1.23.2`，integrity `sha512-OBTsG0W8ddBVOeVVVychpVBS87A9YV5sa2hJ6lc025T97Le+J4v++PwSC4XFs1C62SWyNdof0Mh4KvnZgtt4aw==`。这是 npm 最后一个同时携带 macOS arm64、macOS x64 与 Windows x64 N-API v6 二进制的冻结版本；三端插件打包门禁会逐一验证目标文件格式。linux `napi-v6` 仅有 x64/arm64，没有 loong64。UOS 20 不把 MOSS 标成可启用能力（见 `docs/0.6.0/MOSS_LOONG64.md`）。
