...
- **不引入新依赖、不预设外部 CLI、不执行抓回的内容**；外部 API **只用免费的**（公开免 key，或免费登录/免费 key，如高德）；
- 提炼原技能的**方法论**，删掉一切品牌名/外站 URL 载荷/可执行注入；中文化。
- 形态梯度：能用纯 SOP 表达就别造工具（参照 memory/penglai_weather_sop.md 是 SOP 不是工具）。

## ★选品铁律：必须免费可用

集市技能、以及任何接入的 MCP server，**用户必须能免费用上**——这是入选的硬门槛：
- ✅ 零凭证（纯指导 SOP / Open-Meteo 天气 / web_search 等）；
- ✅ **免费登录或免费 key**（如高德开放平台免费 key、邮箱用户自己的账号、Agnes 免费 API）——登录/注册可以；
- ❌ **任何「必须付费才能用」的一律不收**：付费订阅 / 付费 API / 按量计费才出结果（付费图像视频生成、付费股票行情、电话短信 API、付费套餐专属模型等）。
- 判据：一个没花钱的普通用户，注册个免费账号后能不能**完整用上**？不能 = 不收。
