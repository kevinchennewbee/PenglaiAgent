window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		const WELCOME_NOTICE_VERSION = "penglai-0.5.0.2";
		/** The complete editable internal-testing notice in both supported GUI locales. */
		const WELCOME_NOTICE_COPY = {
			zh: {
				title: "欢迎使用蓬莱",
				body: "欢迎使用蓬莱 0.5.0。对话、工作区、模型和插件都在这一个窗口里完成。\n\nAPI 密钥和即时通讯凭据只保存在本机 YAML 文件，不会上传到蓬莱云。请不要关闭系统安全提示。关于页可以查看引擎版本和开源归属。",
				continueLabel: "开始使用"
			},
			en: {
				title: "Welcome to Penglai",
				body: "Welcome to Penglai 0.5.0. Chat, workspaces, models, and plugins all live in this window.\n\nAPI keys and messaging credentials stay in a local YAML file on this computer. Penglai does not upload them to a Penglai cloud. Do not turn off system security warnings. About keeps the engine version and open-source attribution.",
