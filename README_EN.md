...
- **2026-06-16** — v0.2.8: **Feishu human-intervention and macOS runtime closeout** — `ask_user` no longer shows only the dangling final sentence in Feishu: structured questions and choices render as clickable buttons, with numbered/exact-text replies as a fallback; Feishu now starts through the Penglai wrapper `penglai_feishu_app.py`, fixing `/review` routing while keeping upstream `frontends/fsapp.py` untouched; the macOS Feishu frontend is now under **launchd supervision** too (legacy naked processes are cleaned up during migration, and crashes/kills reconnect automatically); the WeChat wrapper gains `/new` / `/restore` / `/continue` / `/btw` / `/review`; `fs_owner_open_id` is separated from the access allowlist so owner notifications no longer depend on an open allowlist; doctor now reports the real `.venv` Python and launchd state. Verified on Tencent Cloud and Mac mini, **zero GA-kernel diff**.
