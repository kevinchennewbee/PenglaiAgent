...
test("beginWeixinQr returns qrImageRef and production pack scripts require --target", async () => {
  const rt = createRuntime({
    dbPath: ":memory:",
    host: { version: "0.1.2-rc.1", getAgent: () => undefined, listWorkspaces: () => [{ id: "w", title: "W", sessionIds: ["s"] }] },
  });
  const weixin = {
  ...
  const host = new PenglaiImHost(
    ...
    { version: "0.1.2-rc.1", getAgent: () => undefined, listWorkspaces: () => [{ id: "w", title: "W", sessionIds: ["s"] }] },
  );
