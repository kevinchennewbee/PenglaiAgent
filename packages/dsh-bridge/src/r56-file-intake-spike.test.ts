...
test("R56-FILE-016 official DSH rc.2 file intake spike is BLOCKED", () => {
  ...
  assert.deepEqual(report.officialImageMediaTypes, [...OFFICIAL_IMAGE_MEDIA_TYPES]);
  ...
  assert.deepEqual(report.conversationInputSlots, [...OFFICIAL_CONVERSATION_INPUT_SLOTS]);
  assert.deepEqual(report.composerDraftFields, ["draft", "imageIds"]);
  assert.deepEqual(report.genericFileApis, []);
  assert.ok(report.notes.some((note) => note.includes("Do not bind ordinary files")));
});

test("R56-FILE-016 refuses unofficial file-to-Turn binding", () => {
  assert.throws(() => refuseUnofficialFileTurnBinding(), (error: unknown) => {
