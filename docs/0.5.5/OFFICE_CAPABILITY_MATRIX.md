...
| PDF | pdf-lib + bundled OFL CJK | metadata + page count + extracted literals | `pdf.watermark` / rotate / merge | Poppler `pdfinfo`/`pdftotext` | CJK body uses hashed `PenglaiCjkOfl.ttf` (OFL-1.1). Not a Word-style paragraph editor |
