...
function writePluginCatalog(pluginsDir, extra = {}) {
  ...
  const memory = "penglai-memory-0.6.6.tgz";
...
test("UOS installer name is packager-owned and in RELEASE_TARGETS", () => {
  assert.equal(uosDebInstallerName(), "Penglai_0.6.6_uos_loong64.deb");
  assert.equal(UOS_DEB_INSTALLER_NAME, "Penglai_0.6.6_uos_loong64.deb");
  assert.equal(RELEASE_TARGETS.includes("linux-loong64"), true);
  assert.equal(TARGET_INSTALLERS["linux-loong64"], "Penglai_0.6.6_uos_loong64.deb");
});
...
test("staged .deb keeps /opt/Penglai, desktop file, and required Memory", () => {
  ...
  try {
    ...
    assert.equal(control.Version, "0.6.6");
    ...
    for (const excluded of ["office"]) {
      assert.equal(data.has(`opt/Penglai/resources/plugins/penglai-${excluded}-0.6.6.tgz`), false);
    }
    assert.equal(
      data.has("opt/Penglai/resources/plugins/penglai-memory-0.6.6.tgz"),
      true,
    );
    ...
    writePayload(disabled, {
      catalog: {
        entries: [
          {
            id: "@penglai/memory",
            packageFile: "penglai-memory-0.6.6.tgz",
