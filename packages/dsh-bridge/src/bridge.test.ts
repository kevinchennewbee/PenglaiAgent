...
test("R1-UP-001 pinned official packages are 0.1.5-rc.2", () => {
  const pinned = probePinnedPackages();
  assert.equal(pinned.dsh, "0.1.5-rc.2");
  assert.equal(pinned.agent, "0.1.5-rc.2");
  assert.equal(pinned.llm, "0.1.5-rc.2");
  assert.equal(pinned.workspace, "0.1.5-rc.2");
});
