...
Status: active 0.6.3 release procedure. Native results become PASS only when
...
The 0.6.2 to 0.6.3 installed-upgrade journey is `OWNER_EXCLUDED` for this
release. `UPGRADE_SOURCES.json` retains immutable historical pins for updater
identity and future historical verification, but the 0.6.3 native workflow must
...
默认卸载，并证明 Owner 数据保留。0.6.2 到 0.6.3 的安装升级在本版明确为
