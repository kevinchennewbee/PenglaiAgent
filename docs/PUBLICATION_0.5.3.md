...
Post-publication outcome for item 4: **not satisfied as a fresh-profile UI claim**. The public 0.5.1 updater discovered v0.5.3 and selected the exact Apple Silicon asset, but the already-published 0.5.1 completion gate expects `$DSH_HOME/workspace.json` while DSH rc.1 writes `storages/workspace.json`. A fresh profile therefore remains at wizard step 7 and cannot reach the visible Settings update page. The 0.5.3 Release remains valid for fresh installs and manual same-platform overlays, and completed 0.5.1 profiles retain their signed updater path; documentation must not claim that the old binary gained a recovery button.
