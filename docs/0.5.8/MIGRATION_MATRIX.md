...
| Audio intake | Penglai ASR before a text Turn | No general DSH audio/video Turn support | Keep audio pipeline in ASR plugin; persist closed downloading/validating/transcoding/transcribing phases plus queued handoff while retaining legacy `processing` recovery | Rebind produced text through migrated Session path | Real Feishu voice becomes one official text Turn; no disguised attachment |
