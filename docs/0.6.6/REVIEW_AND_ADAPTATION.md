...
7. **Fixed in the upgrade verifier, awaiting renewed installed proof:** official 0.1.7 imports legacy `settings.yaml` into the profile and retains the source bytes as `settings.yaml.imported`. The installed check now requires the original settings, exact imported archive, consumed legacy path and applied locale preference; the old assertion expected a live target `settings.yaml` and falsely reported data loss.
8. **Still required:** native installed onboarding and plugin enable/disable/restart, real 0.6.3/0.6.5-to-0.6.6 migration, third-party manager consent, Weixin/Feishu live-account delivery where credentials are available, Memory-real on a clean source SHA, three target artifacts, and immutable public-byte readback. UOS package/ABI evidence is distinct from UOS native-machine evidence. Public README/site must remain on 0.6.5 until readback.
