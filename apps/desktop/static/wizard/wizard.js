(() => {
  const COPY = {
    zh: {
      ...
      languageBody: "先选中文或 English，并选外观。默认中文、跟随系统，之后仍可在设置里改。引导共七步，走到模型真正回复才算完成。",
      privacyTitle: "隐私说明",
      privacyBody:
        "你的 API 密钥和消息平台凭据只保存在这台电脑的蓬莱数据目录中，界面不会显示已保存的完整密钥。与你使用同一电脑账户运行的其他程序，也可能访问本机文件。\n\n没有蓬莱账号，也没有蓬莱云同步。模型调用仍会把这次任务需要的内容发给你选择的供应商。消息平台只转发你明确绑定的私聊。二维码、聊天内容和密钥不会写入诊断日志。",
      modelsTitle: "选择模型",
      modelsBody: "从已配置的供应商里选一个默认模型。列表与设置里的模型页相同。选好后下一步会测试密钥。",
      provider: "供应商",
      model: "模型",
      credentialTitle: "输入 API 密钥",
      credentialBody: "密钥只保存在这台电脑上。继续后，蓬莱会发送一次测试请求。如果失败，可以改密钥后重试，或返回上一步。",
      apiKey: "API 密钥",
      workspaceTitle: "工作区",
      workspaceBody: "选择一个本机文件夹保存工作文件。不能使用蓬莱数据目录或应用安装目录。选错了可以返回重选。",
      workspacePath: "文件夹",
      workspaceTitleField: "名称",
      workspacePick: "选择文件夹",
      workspaceExisting: "已有工作区",
      workspaceNone: "没有已有工作区，请选择文件夹。",
      workspaceEmpty: "尚未选择文件夹",
      firstTurnTitle: "第一条消息",
      firstTurnBody: "发送一条测试消息，确认模型和工作区都能正常使用。只有收到真实回复后才会进入蓬莱。失败可以重试。",
      firstTurnMessage: "消息",
      doneTitle: "可以开始使用",
    ...
    en: {
      ...
      languageBody: "Choose Chinese or English, then pick an appearance. Chinese and system appearance are the defaults. You can change both later in settings. There are seven steps; setup finishes only after a real model reply.",
      privacyTitle: "Privacy",
      privacyBody:
        "Your API keys and messaging credentials stay in Penglai's data folder on this computer. The app does not show a complete saved key. Other programs running under the same computer account may also be able to access local files.\n\nThere is no Penglai account and no Penglai cloud sync. Model calls still send the context needed for that task to the provider you chose. Messaging platforms forward only the private chats you explicitly bind. QR codes, chat contents, and secrets never enter diagnostic logs.",
      modelsTitle: "Choose a model",
      modelsBody: "Pick a default model from the configured providers. This is the same list as the Models page in settings. The next step tests your key.",
      provider: "Provider",
      model: "Model",
      credentialTitle: "Enter API key",
      credentialBody: "The key stays on this computer. Continue sends one test request. If it fails, you can change the key and retry, or go back.",
      apiKey: "API key",
      workspaceTitle: "Workspace",
      workspaceBody: "Choose a local folder for your work files. Penglai will not use the app data directory or the install directory. If the folder is rejected, go back and choose another.",
      ...
      workspaceEmpty: "No folder chosen yet",
      firstTurnTitle: "First message",
      firstTurnBody: "Send a test message to make sure the model and workspace both work. Penglai opens only after a real reply. You can retry if it fails.",
  ...
  function bodyFor(screen) {
    ...
    if (screen.id === "workspace") {
      return [
        ...
        el("p", { className: state.workspacePath ? "path" : "path empty", "data-penglai-wizard-workspace": "1" }, [
          state.workspacePath || t("workspaceEmpty"),
        ]),
        el(
          "button",
          {
  ...
  function render() {
    ...
    const errorText = state.busy ? (screen.id === "keytest" ? t("testing") : t("busy")) : state.error;
    const continueLabel = screen.id === "done" ? t("finish") : state.error ? t("retry") : t("continue");
    ...
    root.replaceChildren(
      ...
      el("div", { className: "wizard-stage" }, [
        ...
        el("p", { className: "step-caption" }, [t(captions[screen.id])]),
        el("section", { className: "card", "data-penglai-wizard-step": screen.id }, [
          ...
          el(
            "p",
            {
              className: errorText && !state.busy ? "error has-error" : "error",
              "data-penglai-wizard-error": "1",
              role: "status",
              "aria-live": "polite",
            },
            [errorText],
