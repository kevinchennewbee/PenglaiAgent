...
└─ official DSH host + official DSH Web
   ...
   └─ Penglai DSH plugins
      ├─ plugin-center
      ├─ memory + Mnemon ──────┐
      ├─ im ─ nine adapters ───┴─ scoped Artifact Service / Owner Broker
      ├─ asr / moss-tts
...
- provides the official events used by Memory (`turn/end`, `agent/pre-step`), IM
  Turn submission, and settings surfaces.
...
archives, memory source documents, and all inbound messages
...
The complete official npm cohort contains 307 verified packages pinned to
`dsh-v0.1.6-alpha.2` at `ddefc45fbc7f8e46dd73185e68295696d1297887`.
The upstream audit ledger records the six-package `@deepseek-ai/libreoffice-kit`
family and Office/PDF packages, but the product closure explicitly prunes them.
Official DSH bytes are unmodified; Penglai composes official client slots.
Session recovery consumers use official Session Controller inspection and fail on missing capability.
Previous Home generations are copied into `0.1.6-alpha.2` before a health-verified
...
### 7. Artifact Service and excluded document scope
