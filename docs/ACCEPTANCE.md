...
**当前 0.6.6：**下文的 80 项 Hard ID 继续由机器解析和原生证据发射，0.6.6 发布时全部 80/80 通过。具体版本范围请以 [0.6.6 验收增量](0.6.6/ACCEPTANCE_DELTA.md)、[当前合同](0.6.6/CURRENT_CONTRACT.md)和[发布清单](PUBLICATION_MANIFEST_0.6.6.md)为准。下文有关“预算与主动陪伴缺席”和 0.6.3→0.6.5 升级排除的解释记录的是 0.6.5 历史范围，不能用于解释 0.6.6；不要修改既有 Hard ID 以制造新版本 PASS。
