...
- **0.6.1 全流程授权与完成记录**（D-071/D-074，目标集合由 D-073 固定）：官方 DSH `0.1.5-rc.1` 完整 279 包 cohort、第一方/伴随插件适配、确定性门禁、三目标构建、不可变发布、README/官网与公开回读已经完成。两小时测试与旧版本已安装升级为该版 `OWNER_EXCLUDED`；UOS 真机安装/启动/功能仍为 `OWNER_POST_RELEASE`。精确冻结与证据边界见 `docs/0.6.1/`。
- **0.6.1 三目标发布**（D-073，已完成）：Owner 指令「不管 intel芯片的，赶紧发布」排除 Intel Mac。0.6.1 精确目标为 `darwin-aarch64`、`win32-x86_64`、`linux-loong64`；精确附件十项。不得把 native34542847487 Intel 失败标 PASS，也不得为 Intel 开 ad hoc 发布旁路。
- **0.6.2 上游复核与隐私维护**（D-075）：2026-09-12 检查到官方 DSH
  `0.1.5-rc.2` 仍位于 npm `next`，相对 rc.1 是反馈弹窗与文件展示类更新，不含
  已识别的核心安全/会话修复；当时决定不启动 0.6.2，后由 D-076 明确取代。Penglai 继续硬禁用 DSH
