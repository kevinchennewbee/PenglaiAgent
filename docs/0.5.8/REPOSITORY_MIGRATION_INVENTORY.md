...
| --- | --- | --- | --- | --- |
| `packages/channel-feishu/src/index.ts` callback/media boundary | Active product | P0-02/P0-03 | Terminal rejection containment, exact capability checklist, durable text-only degradation, and closed request/stream/validation/admission/transcription diagnostics are implemented; arbitrary vendor detail is rejected from audit | Rejected work cannot escape; resource access is not inferred from text connection; retry class and closed cause are durable; Owner-live permission/download evidence remains |
