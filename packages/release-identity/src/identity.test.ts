...
test("R50-TRUTH-001 identity pins are 0.5.7", () => {
  ...
  assert.equal(checked.productVersion, "0.5.7");
  recordAssertion({
    acceptanceId: "R50-TRUTH-001",
    runnerId: "release-identity.identity",
    testId: "identity-pins-0.5.7",
    assertionId: "productVersion-is-0.5.7",
    status: "PASS",
...
test("R50-TRUTH-008 publication fields match the owner-authorized public target", () => {
  ...
  assert.equal(id.publication.tag, "v0.5.7");
  assert.equal(id.publication.release, "v0.5.7");
  assert.equal(id.publication.channel, "stable-v0.5.7");
  assert.throws(
    () =>
      assertReleaseIdentity({
