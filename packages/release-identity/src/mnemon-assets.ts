...
export const MNEMON_UPSTREAM = Object.freeze({
  ...
  hostAllowlist: ["github.com", "objects.githubusercontent.com", "release-assets.githubusercontent.com"] as const,
});
...
export const MNEMON_ASSETS: readonly MnemonAssetPin[] = Object.freeze([
  {
...
export function mnemonReleaseUrl(filename: string): string {
  return `https://github.com/${MNEMON_UPSTREAM.owner}/${MNEMON_UPSTREAM.repo}/releases/download/${MNEMON_UPSTREAM.tag}/${filename}`;
}

export function mnemonAssetForTarget(target: string): MnemonAssetPin | undefined {
  return MNEMON_ASSETS.find((row) => row.target === target);
}

export function mnemonAssetForHost(platform = process.platform, arch: string = process.arch): MnemonAssetPin | undefined {
  const normalized = arch === "arm64" || arch === "x64" ? arch : "other";
  return MNEMON_ASSETS.find((row) => row.platform === platform && row.arch === normalized);
}

export function mnemonAssetForPluginTarget(pluginTarget: string): MnemonAssetPin | undefined {
