...
> 当前 0.6.3 在本合同的 R50/R55 能力类别基础上增加
> `docs/0.6.3/ACCEPTANCE_DELTA.md`。本文保留历史机器可解析 Hard ID，精确版本、
> 文件名与公开目标以 `release-contract.json` 为准；任何旧版本字样都不能覆盖当前
> 0.6.3 机器身份。已发布的 0.5.10、0.5.11、0.5.12、0.6.0、0.6.1 与 0.6.2 验收增量保持不可变。
> 0.6.3 明确排除 LibreOffice、Office/PDF、Budget 与 Companion；下文对应的
> 历史 R50/R55 条目对 0.6.3 为 `NOT_APPLICABLE`，不得据此把已排除模块重新打包。
> 当前适用范围以 `docs/0.6.3/ACCEPTANCE_DELTA.md` 为准。

## 1. 判定对象与结论

本文的 R50/R55 表格记录 0.5.x 基础能力，不再单独定义当前 release set。当前唯一
验收对象由 `release-contract.json` 与 `docs/0.6.3/ACCEPTANCE_DELTA.md` 定义：同一
clean source 与 deterministic public-export tree 产生 Apple Silicon、Windows x64、
UOS loong64 三个安装包及七项元数据。Intel Mac 不属于 0.6.3 精确集合。交叉构建
...
以下每一行都是对应证据类别内的 Hard assertion。registry 继续保留 R50/R55 的机器可解析 ID，并由 0.5.11 delta 增加本轮产品门；基础表预期共 **330** 个唯一 ID。实现必须动态解析，不能把计数写成散落的完成映射。每个ID必须指向真实runner的具体assertion，包含candidate/source/export/target/artifact/runner native/时间/exit/result digest。不能通过文件名、字符串存在或一个smoke扇出PASS。需要所有者账号的 live/soak assertion 属补充验收：缺失不能冒充 PASS，也不作为自动化发布聚合的永久阻塞项。

平台标记：

- `all`：平台类门禁展开为全部三个目标；非平台类门禁只执行一次源码/聚合检查。
- `mac-arm`、`mac-x64`、`win-x64`：分别绑定三个 exact native artifact，均参与 0.5.11 PASS。
- `live`：真实账户最后集中执行。
...
| ID | 要求 | Runner |
| --- | --- | --- |
| `R50-TRUTH-001` | root/workspace/desktop/profile/plugin/release contract版本全部为当前 `release-contract.json` 的 0.5.11 | contract/all |
...
| `R50-TRUTH-008` | repo=`kevinchennewbee/PenglaiAgent`、tag/release=`v0.5.11`，且发布前 updater Release 不得冒充已公开 | manifest/aggregate |
...
| `R50-UI-007` | About 显示 0.5.11、DSH/target/trust/data/license 准确 | installed/all |
...
| `R50-UPD-012` |三个 native target 的 0.5.11→test-next valid/failure suites PASS，且 Apple Silicon 受支持旧版→公开版 0.5.11 实装升级 PASS | installed/all |
...
| `R50-REL-010` |历史长时稳定性项；0.5.11 按 Owner 指令 NOT_APPLICABLE，不运行、不作为待办；正常有限负载回归另行执行 | soak/all |
...
### R55. 0.5.5 产品增量（作为 0.5.11 基础继续执行）

#### Truth / DSH（8）

| ID | 要求 | Runner |
| --- | --- | --- |
| `R55-TRUTH-001` | all product/package/release versions exact current 0.5.11 | contract/all |
...
- 未经 Owner 明确授权修改开源仓库或发布；0.5.11 已获授权，但任一必需门禁失败仍一票否决。已发布的 0.5.10 不得改写。
