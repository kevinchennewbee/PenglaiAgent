...
test("R50-UPD-007 schema backup is app-private bounded and excludes credentials/workspaces/voice audio", () => {
  ...
  const alpha2Home = join(root, "dsh-homes", "dsh-v0.1.3-alpha.2");
  ...
  assert.equal(existsSync(join(result.path, "dsh-homes", "dsh-v0.1.3-alpha.2", "settings.yaml")), true);
  assert.equal(existsSync(join(result.path, "dsh-homes", "dsh-v0.1.3-alpha.2", ".credentials.yaml")), false);
  assert.equal(existsSync(join(result.path, "dsh-homes", "dsh-v0.1.3-alpha.2", "storages", "workspace.json")), false);
