...
fn find_project_python(root: &Path) -> Option<String> {
    #[cfg(windows)]
    {
...
fn packaged_runtime_dir(app_handle: &tauri::AppHandle) -> Option<PathBuf> {
    let candidates = [
        app_handle
            ...
            .resource_dir()
            .ok()
            .map(|p| p.join(PACKAGED_RUNTIME_RESOURCE)),
        app_handle
            .path()
            .resource_dir()
            .ok()
            .map(|p| p.join("resources").join(PACKAGED_RUNTIME_RESOURCE)),
        std::env::current_exe()
            .ok()
        ...
        Some(
            project_root()
                ...
                .join("src-tauri")
                .join("resources")
                .join(PACKAGED_RUNTIME_RESOURCE),
        ),
    ];
    candidates.into_iter().flatten().find(|p| {
        p.join(PACKAGED_RUNTIME_MANIFEST).exists() && p.join("source").join("penglai").exists()
    })
}
...
fn verify_manifest_python(runtime_dir: &Path, manifest: &RuntimeManifest) -> Result<(), String> {
    ...
    if !matches!(python_kind, "standalone" | "embedded" | "venv") {
        return Err(format!(
            "包内 Python manifest python_kind 不支持: {}",
            python_kind
        ));
    }
    ...
    if !matches!(python_scope, "runtime" | "source") {
        return Err(format!(
            "包内 Python manifest python_scope 不支持: {}",
            python_scope
        ));
    }
    ...
    if !manifest
        .files
        .iter()
        .any(|item| item.path == manifest_python_file)
    {
        return Err(format!(
            "包内 Python 未纳入 SHA-256 文件清单: {}",
            manifest_python_file
        ));
    }
    if manifest
        .python_version
    ...
    for dep in dependency_lock {
        if dep.version.trim().is_empty() {
            return Err(format!(
                "包内 Python dependency_lock 缺少版本号: {}",
                dep.name
            ));
        }
...
fn manifest_python_path(
    ...
    let python_scope = manifest.python_scope.as_deref().unwrap_or("").trim();
    let root = if python_scope == "runtime" {
        runtime_dir
    } else {
        project
    };
...
fn verify_packaged_runtime(runtime_dir: &Path) -> Result<RuntimeManifest, String> {
    ...
    if manifest.schema != 1 || manifest.kind != "penglai-desktop-runtime" {
        return Err(format!(
            "包内运行时 manifest 类型不匹配: schema={} kind={}",
            manifest.schema, manifest.kind
        ));
    }
    ...
    if let Some(platform) = &manifest.platform {
        ...
        if actual_os != expected_os {
            return Err(format!(
                "包内运行时平台不匹配: expected {} got {}",
                expected_os, platform.os
            ));
        }
    ...
    for item in &manifest.files {
        ...
        let meta =
            fs::metadata(&path).map_err(|e| format!("包内运行时文件缺失 {:?}: {}", path, e))?;
        if !meta.is_file() {
        ...
        if actual != item.sha256 {
            return Err(format!(
                "包内运行时 SHA-256 不匹配 {:?}: expected {} got {}",
...
fn runtime_payload_selfcheck() -> Result<(), String> {
    let runtime_dir = packaged_runtime_dir_from_exe().ok_or_else(|| {
        "installed runtime payload not found next to desktop executable".to_string()
    })?;
    ...
    println!(
        "{}",
        serde_json::to_string(&payload).unwrap_or_else(|_| "{\"ok\":true}".to_string())
    );
    Ok(())
}
...
fn copy_dir_recursive(src: &Path, dst: &Path) -> Result<(), String> {
    ...
    for entry in fs::read_dir(src).map_err(|e| format!("read {:?}: {}", src, e))? {
        ...
        let file_type = entry
            .file_type()
            .map_err(|e| format!("file type {:?}: {}", entry.path(), e))?;
...
fn prepare_packaged_runtime(runtime_dir: &Path, project: &Path) -> Result<String, String> {
    ...
    if !is_penglai_runtime_dir(&source_dir) {
        return Err(format!(
            "包内运行时缺少 source/penglai 或 source/agent_loop.py: {:?}",
            runtime_dir
        ));
    }
    ...
    if !is_penglai_runtime_dir(project) {
        ...
        if fs::symlink_metadata(&tmp)
            ...
            .unwrap_or(false)
        {
            return Err(format!("临时运行时目录不能是符号链接: {}", tmp.display()));
        }
        if tmp.exists() {
        ...
        if project.exists() {
            fs::remove_dir_all(project)
                .map_err(|e| format!("清理空运行时目录失败 {:?}: {}", project, e))?;
        }
    ...
    let python_path = manifest_python_path(runtime_dir, project, &manifest)
        .or_else(|| find_project_python(project))
...
fn generate_bridge_token() -> String {
    ...
    let digest =
        Sha256::digest(format!("{}:{}:{:?}", now, pid, std::env::current_exe()).as_bytes());
...
fn start_bridge() {
    ...
    cmd.arg(&script).current_dir(script.parent().unwrap());
    ...
    match cmd.spawn() {
        ...
        Err(e) => {
            eprintln!(
                "[penglai-desktop] failed to start bridge: {} (python={})",
                e, python
            );
            return;
        }
...
fn start_bridge_with_config(
    ...
    if !is_bridge_running() {
        if is_bridge_port_open() {
            return Err(
                "检测到旧版或非蓬莱桌面桥接占用 127.0.0.1:14168；请先关闭旧桥接后重试。".into(),
            );
        }
...
fn install_runtime(
    ...
    let runtime_dir = packaged_runtime_dir(&app_handle).ok_or_else(|| {
        "安装包缺少内置 penglai-runtime；发布版不会下载并执行远程安装脚本。".to_string()
    })?;
    let python_path = install_from_packaged_runtime(&app_handle, &runtime_dir, &project)?;
    ...
    if !is_bridge_running() {
        ...
        if !script.exists() {
            return Err(format!(
                "desktop_bridge.py not found at {:?} after install",
                script
            ));
        }
        let mut cmd = Command::new(&python_path);
...
#[tauri::command]
fn install_runtime_step(app_handle: tauri::AppHandle, step: String) -> Result<String, String> {
...
fn check_update() -> Result<UpdateInfo, String> {
    ...
    let old_version = extract_field(&text, "当前")
        .or(extract_field(&text, "old"))
        .unwrap_or_default();
    let new_version = extract_field(&text, "最新")
        .or(extract_field(&text, "new"))
        .unwrap_or_default();
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let show_hide_item =
                tauri::menu::MenuItemBuilder::with_id("show_hide", "显示主窗口").build(app)?;
            let check_update_item =
                tauri::menu::MenuItemBuilder::with_id("check_update", "检查更新").build(app)?;
            let quit_item = tauri::menu::MenuItemBuilder::with_id("quit", "退出蓬莱").build(app)?;
            ...
            let bridge_ready = if bridge_ok {
                true
            } else {
                wait_for_bridge_ready(bridge_wait)
            };
            if bridge_ready {
                if let Some(w) = app.get_webview_window("main") {
                    let url = tauri::Url::parse("http://127.0.0.1:14168/").unwrap();
