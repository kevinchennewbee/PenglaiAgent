...
try {
  ...
  const created = await evaluate(session, rpcExpression("createWorkspace", { path: workspace, title: `Penglai ${PRODUCT_VERSION} Live` }));
  ...
  const firstMessage = `Penglai ${PRODUCT_VERSION} live verification ${randomUUID().slice(0, 8)}. Reply with OK.`;
