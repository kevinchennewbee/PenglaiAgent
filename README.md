...
- 🧠 **四层记忆** —— 基于 GA 内核的索引/事实/技能/原始会话四层文件式记忆，纯 markdown 可审计；写入前威胁扫描（提示注入/角色劫持/密钥落库），禁止覆盖
- 🛡️ **确定性安全护栏** —— 危险命令与路径红线拦截 + 全量工具调用审计 JSONL——**靠确定性检查，不靠 LLM 自觉**。已覆盖危险命令、敏感路径、记忆写入、文件外发（白名单当前仅覆盖飞书渠道）等关键风险面；护栏 ≠ 绝对安全，建议在个人受控的服务器上运行
- 🧐 **防幻觉双保险** —— 本地绊线**出厂常开**（免费）：嗅到「过度自信」措辞就拦下自检；`penglai enable critic` 一条命令再配上**另一厂商**的免费模型（如 GLM-4.7-Flash）做交叉复核——单模型查不出自己的幻觉；求证型任务可开多源搜索交叉验证
...
| 批判脑 <sub>smart 档</sub> | hook | 绊线常开（免费）；命中才异厂商复核（`penglai enable critic`） |
