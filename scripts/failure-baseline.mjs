...
function probeVersions() {
  ...
  const ok = root.version === "0.5.0" && info.productVersion === "0.5.0";
  if (!ok) {
    return result("FB-VERSIONS", "REPRODUCED", "workspace/release-info not 0.5.0", {
      packageVersion: root.version,
      productVersion: info.productVersion,
    });
  }
  return result("FB-VERSIONS", "CLOSED", "root and release-info are 0.5.0", { version: root.version });
}
...
function probePublication() {
  ...
  const ok =
    ...
    pub.tag === "v0.5.0" &&
    pub.release === "v0.5.0" &&
    pub.channel === "NOT_PUBLISHED_0_5_0";
  if (!ok) {
...
writeFileSync(
  ...
  JSON.stringify({ schema: 3, version: "0.5.0", probes: out, mustClose: MUST_CLOSE }, null, 2),
);
console.log(
  "failure-baseline",
