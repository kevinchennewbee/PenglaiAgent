...
Docker 安装网络兜底热修：Docker 一键部署在 GHCR 拉取失败或超时时，会进入国内镜像站或本地源码构建；本地构建也会同步切到国内 PyPI/APT 镜像，可用 `PENGLAI_DOCKER_PULL_TIMEOUT`、`PENGLAI_PIP_INDEX`、`PENGLAI_APT_MIRROR` 调整。

近期更新重点：

- v0.2.26：Docker 拉取超时热修，避免 GHCR 长时间无响应时卡住，进入后续兜底路径。
