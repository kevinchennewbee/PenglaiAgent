...
test("R50-UPD-007 schema backup is app-private bounded and excludes credentials/workspaces/voice audio", () => {
  ...
  const alpha2Home = join(root, "dsh-homes", "dsh-v0.1.5-alpha.1");
  ...
  assert.equal(existsSync(join(result.path, "dsh-homes", "dsh-v0.1.5-alpha.1", "settings.yaml")), true);
  assert.equal(existsSync(join(result.path, "dsh-homes", "dsh-v0.1.5-alpha.1", ".credentials.yaml")), false);
  assert.equal(existsSync(join(result.path, "dsh-homes", "dsh-v0.1.5-alpha.1", "storages", "workspace.json")), false);
