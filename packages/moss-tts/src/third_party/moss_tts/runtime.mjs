/*
 * Derived from OpenMOSS MOSS-TTS-Nano-Reader's browser ONNX runtime.
 * Copyright 2026 OpenMOSS Team, Fudan University, SII and MOSI.
 * Licensed under Apache-2.0; see ./LICENSE.
 *
 * Modified by PenglaiAgent for the Node.js host: onnxruntime-node CPU
 * sessions, filesystem-backed model loading, and a native Node
 * SentencePiece adapter replace the browser WASM / iframe dependencies.
 */
...
const { SentencePieceProcessor } = sentencepiece;
...
function resolveLocalAssetPath(rootPath, relativePath) {
  const root = path.resolve(normalizeLocalDirectoryPath(rootPath));
  const normalizedPath = normalizeRelativePath(relativePath);
  if (!normalizedPath || path.isAbsolute(normalizedPath) || normalizedPath.includes("\0")) {
    throw new Error("Unsafe local MOSS asset path.");
  }
  const resolved = path.resolve(root, normalizedPath);
  const relative = path.relative(root, resolved);
  if (!relative || relative === ".." || relative.startsWith(`..${path.sep}`)) {
    throw new Error("Local MOSS asset escaped the verified model root.");
  }
  return resolved;
}
async function readTextFromLocalPath(rootPath, relativePath) {
  return readFile(resolveLocalAssetPath(rootPath, relativePath), "utf8");
}
async function readBufferFromLocalPath(rootPath, relativePath) {
  return new Uint8Array(await readFile(resolveLocalAssetPath(rootPath, relativePath)));
}
...
var BrowserOnnxTtsRuntime = class {
  ...
  getEffectiveThreadCount() {
    return Math.max(1, this.threadCount);
  }
  ...
  async ensureOrtConfigured() {
    // onnxruntime-node selects the local CPU execution provider.  Thread
    // controls belong to native SessionOptions, not the browser WASM env.
  }
  async createOrtSession(relativePath, externalDataRelativePaths = []) {
    // Resolve through the same guarded helper as readFile so a manifest
    // relative path containing ".." cannot escape the verified model root.
    const modelPath = resolveLocalAssetPath(this.localPathRoot, relativePath);
    const sessionOptions = {
      ...ORT_SESSION_OPTIONS,
      executionProviders: [...ORT_SESSION_OPTIONS.executionProviders],
      intraOpNumThreads: this.getEffectiveThreadCount()
    };
    this.log(`Creating native ORT session from path: ${relativePath}`);
    // Passing a filesystem path lets ORT resolve the model's external .data
    // sidecars in place without copying hundreds of megabytes into JS heaps.
  ...
  async ensureTokenizerLoaded() {
    ...
    await tokenizer.load(resolveLocalAssetPath(this.localPathRoot, tokenizerRelativePath));
    this.tokenizer = tokenizer;
    this.tokenizerModelKey = modelKey;
    this.log(`Tokenizer loaded in Node: ${tokenizerRelativePath}`);
  }
  ...
  async ensureSynthesisLoaded() {
    ...
    this.log(
      `ORT node=${ort.env.versions?.node || "unknown"} threads=${this.getEffectiveThreadCount()} provider=cpu`
    );
  ...
  async encodeText(text) {
    ...
    const startedAt = this.profileState?.enabled ? nowMs() : 0;
    try {
      return this.tokenizer.encodeIds(String(text || ""));
    } finally {
