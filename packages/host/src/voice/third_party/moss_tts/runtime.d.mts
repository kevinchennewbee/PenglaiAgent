...
export class BrowserOnnxTtsRuntime {
  constructor(options?: {
    ...
    assetReader?: {
      readText(input: { rootPath: string; relativePath: string }): Promise<string>;
      readJson(input: { rootPath: string; relativePath: string }): Promise<unknown>;
      readBuffer(input: { rootPath: string; relativePath: string }): Promise<Uint8Array>;
    } | null;
  });
...
export function createBrowserOnnxTtsRuntime(options?: ConstructorParameters<typeof BrowserOnnxTtsRuntime>[0]): BrowserOnnxTtsRuntime;
