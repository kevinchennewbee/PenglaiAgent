...
export function assertCanonicalUpdaterManifestUrl(url: string, signature = false): void {
  ...
  const expectedPath = signature
    ? "/kevinchennewbee/PenglaiAgent/releases/download/desktop-v0.5/latest.json.sig"
    : "/kevinchennewbee/PenglaiAgent/releases/download/desktop-v0.5/latest.json";
  if (
    ...
    throw new PenglaiError("SECURITY_POLICY", "updater manifest URL is not canonical desktop-v0.5 HTTPS");
  }
}
...
export function assertReleaseContract(raw: unknown): ReleaseContract {
  ...
  if (pub.tag !== "v0.5.0") throw new PenglaiError("SECURITY_POLICY", "publication.tag");
  if (pub.release !== "v0.5.0") throw new PenglaiError("SECURITY_POLICY", "publication.release");
  if (pub.channel !== "NOT_PUBLISHED_0_5_0") throw new PenglaiError("SECURITY_POLICY", "publication.channel");
