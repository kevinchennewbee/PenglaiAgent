...
- `PRODUCT_CONSTITUTION.md`, `docs/PRODUCT.md`, `docs/ARCHITECTURE.md`,
  ...
  artifact evidence.
- v0.5.10, v0.5.11 and v0.5.12 are immutable public history. For 0.6.0,
  `docs/0.6.0/` and the current source govern development work. They are
  not public-release truth until immutable `v0.6.0` bytes exist, and must
  not modify or weaken published 0.5.10, 0.5.11 or 0.5.12.
...
- For 0.6.0, the Owner-fixed upstream baseline is official npm
  `0.1.5-alpha.1`, tag `dsh-v0.1.5-alpha.1`, commit
  `5dda764ed3aa172535a7967b06ff95d9cbfe536a`. Penglai consumes the complete
...
- 0.6.0 identity, native and publication work lands through `grok/0.6.0`
  onto `main`. The Owner has authorized PR, `main` merge, four-target native
  build, and v0.6.0 publication only after the applicable gates pass. Do not
...
- v0.5.10、v0.5.11、v0.5.12 已不可变。0.6.0 以 `docs/0.6.0/` 与当前源码约束开发；
  在不可变 `v0.6.0` 公网字节存在前它们不是公开发布事实，也不得修改或削弱已经
  不可变发布的 0.5.10、0.5.11 或 0.5.12 历史。
...
- 0.6.0 的 Owner 固定上游基线为官方 npm `0.1.5-alpha.1`，tag
...
- 0.6.0 身份、原生与发布工作经 `grok/0.6.0` 进入 `main`。Owner 已授权在适用
  门禁全部通过后创建 PR、合并 `main`、完成四端原生构建并发布 v0.6.0；不得改写
  既有发布 tag/附件，也不得在公网字节存在前部署 0.6.0 下载声明。
