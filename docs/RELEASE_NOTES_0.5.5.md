...
The immutable signed [`plugin-catalog-v1.000006`](https://github.com/kevinchennewbee/PenglaiPluginRegistry/releases/tag/plugin-catalog-v1.000006) intentionally lists no downloads. It retires the old read-only `@penglai/office-reader` 0.1.3 by exact id, version and archive SHA because 0.5.5 now includes the full Penglai Office plugin. An installed Reader is disabled before the DSH loader starts, while the historical 000005 Release remains immutable. The client verifies the new catalog signature and retains a signed last-good copy for offline startup.
