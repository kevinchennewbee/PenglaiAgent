...
- Pin `libopus-wasm@0.3.0` at `bd37b907c636705d59cc2b836e6912e317a65a47` (MIT, libopus 1.6.1 notices) for raw Opus encode/decode. 0.3.0 keeps `createEncoder`/`createDecoder`/`Application`/`Signal`, requires Node 22+, and rejects `maxPacketBytes` wrap. Its generated module contains the WASM payload, has no install script, and is packaged inside `@penglai/im`. The upstream module contains absolute compiler paths only in debug strings; packaging verifies the upstream hash, replaces that prefix with a same-length neutral prefix, verifies the transformed hash, and instantiates/encodes with the transformed runtime.
