...
test("R50-TRUTH-001 identity pins are 0.6.5", () => {
  ...
  assert.equal(checked.productVersion, "0.6.5");
  recordAssertion({
...
test("R50-TRUTH-008 publication fields match the owner-authorized public target", () => {
  ...
  assert.equal(id.publication.tag, "v0.6.5");
  assert.equal(id.publication.release, "v0.6.5");
  assert.equal(id.publication.channel, "stable-v0.6.5");
  assert.throws(
    () =>
      assertReleaseIdentity({
