...
function oneMatch(source, pattern, label) {
  ...
  if (matches.length !== 1 || matches[0]?.[1] === undefined) {
    throw new Error(`release pin ${label} must have one closed source declaration`);
  }
  return matches[0][1];
}
...
function numberPin(source, name) {
  const value = Number(
    oneMatch(
      source,
      new RegExp(`export const ${name} = ([0-9]+);`, "g"),
      name,
    ),
  );
  if (!Number.isSafeInteger(value)) throw new Error(`release pin ${name} is not a safe integer`);
  return value;
}
...
function objectString(block, owner, key) {
  return oneMatch(block, new RegExp(`\\n  ${key}: "([^"\\n]+)",`, "g"), `${owner}.${key}`);
}
...
function releaseTargets(source) {
  ...
  if (rows.length !== 3 || new Set(rows.map((row) => row.key)).size !== rows.length) {
    throw new Error("release pin RELEASE_TARGETS must contain three unique closed rows");
  }
  return rows;
}
...
export function readReleaseIdentityPins(root = ROOT) {
  ...
  return Object.freeze({
    ...
    targets: Object.freeze(releaseTargets(source).map((row) => Object.freeze(row))),
  });
}
