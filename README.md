...
- **2026-06-17** — v0.2.10：**飞书真实工作流修复**——飞书消息不再因前一条任务运行中直接丢弃，后续消息会进入队列并在当前任务结束后自动处理；飞书按钮选择的无效点击不再吞掉待选状态；模型最终空输出但已生成文件时，卡片会显示“正在发送生成文件”而不是误报截断；飞书日志里的 API key / token / secret 会脱敏；`[FILE:]` 外发不再按目录限制，只拦截 `.py` / `.env` / `.key` / `.sh` / `.pem` 这几个敏感后缀，视频、图片、PDF、Office 文档和其它实际工作产物可从 Downloads 等实际落点外发；视频/音频在飞书里按文件消息发送，避免媒体类型不匹配。全部蓬莱层，GA 上游代码不改。
