...
2. 只改头部"用户配置区"：去 `mykey.py` 里扫描变量名（⚠️ 只看名字，禁止输出 apikey 值），优先使用 `native_oai_config`，否则使用 `*_native_oai_config` 这类主力 OAI 兼容配置填入 `OPENAI_CONFIG_KEY`，`DEFAULT_BACKEND` 选 `openai`，并测试
