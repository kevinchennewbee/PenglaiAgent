...
Penglai 0.6.1 is a **community-verified** desktop distribution of official DeepSeek Harness (DSH) `0.1.5-rc.1`. This file is the public security entry. The full product contract lives in [`docs/SECURITY.md`](docs/SECURITY.md).
...
The 0.6.1 release contains all ten files in its release contract: three
...
the public source-export manifest. Intel Mac is excluded from 0.6.1; published
...
Official DSH 0.1.5-rc.1 bundles a session-telemetry adapter and a configured DeepSeek
...
WhatsApp is not displayed, supported, planned, or bundled in 0.6.1.
...
当前正式版本为 0.6.1，使用固定的官方 DSH `0.1.5-rc.1` npm 包。
Apple Silicon 和 Windows x64 已做全新安装与默认卸载验证。Intel Mac 不是
0.6.1 安装目标；已发布的 0.6.0 保持不可变。UOS 龙芯包已发布，真机安装/启动/功能为 Owner 发布后测试。
旧版 DSH Home 保留，新代际在独立目录通过健康检查后启用。

0.6.1 的正式发布包含十项完整附件，包括三个安装包、签名更新清单、校验和、
