...
function probeVersions() {
  ...
  const ok = root.version === "0.5.9" && info.productVersion === "0.5.9";
  if (!ok) {
    return result("FB-VERSIONS", "REPRODUCED", "workspace/release-info not 0.5.9", {
      packageVersion: root.version,
      productVersion: info.productVersion,
    });
  }
  return result("FB-VERSIONS", "CLOSED", "root and release-info are 0.5.9", { version: root.version });
}
...
writeFileSync(
  ...
  JSON.stringify({ schema: 3, version: "0.5.9", probes: out, mustClose: MUST_CLOSE }, null, 2),
);
console.log(
  "failure-baseline",
