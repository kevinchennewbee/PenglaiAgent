...
function probeVersions() {
  ...
  const ok = root.version === "0.6.2" && info.productVersion === "0.6.2";
  if (!ok) {
    return result("FB-VERSIONS", "REPRODUCED", "workspace/release-info not 0.6.2", {
      packageVersion: root.version,
      productVersion: info.productVersion,
    });
  }
  return result("FB-VERSIONS", "CLOSED", "root and release-info are 0.6.2", { version: root.version });
}
...
writeFileSync(
  ...
  JSON.stringify({ schema: 3, version: "0.6.2", probes: out, mustClose: MUST_CLOSE }, null, 2),
);
console.log(
  "failure-baseline",
