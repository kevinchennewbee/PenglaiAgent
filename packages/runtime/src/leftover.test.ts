...
test("R50-UPD-008/009/010 verified installer and crash replay", () => {
  ...
  const path = join(root, "Penglai_0.5.10_macos_aarch64.dmg");
...
test("verified installer handoff refuses a symlink even when target bytes are signed", () => {
  ...
  const linked = join(root, "Penglai_0.5.10_macos_aarch64.dmg");
  const payload = Buffer.from("signed-installer");
...
test("R50-UPD: download verifies size/hash/signature and crash mid-download returns IDLE", async () => {
  ...
  const out = await downloadVerifiedPayload({
    url: "https://github.com/kevinchennewbee/PenglaiAgent/releases/download/v0.5.10/Penglai_0.5.10_macos_aarch64.dmg",
    destDir: dest,
    expectedSha256: sha,
...
test("R50-DIST: packaged identity is Penglai 0.5.10 and Windows NSIS stays current-user", async () => {
  const {
  ...
  assert.equal(facts.shortVersion, "0.5.10");
