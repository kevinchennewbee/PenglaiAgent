...
test("R2I-CRED-009 feishu secret write/read via host round-trips without plaintext in overview", async () => {
  ...
  const host = new PenglaiImHost(rt.store, rt.plane, { health: () => ({ authState: "idle", hasCredential: false }) } as never, feishu as never, vault, { running: false, start: async () => undefined, stop: () => undefined } as never, { version: "0.1.0-rc.8", getAgent: () => undefined, listWorkspaces: () => [] });
