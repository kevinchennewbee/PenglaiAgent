#!/usr/bin/env node
/**
 * sync-catalog.mjs — 从 catalog.source.yaml 生成 TS 供应商目录数据源。
 *
 * 三层新鲜度机制（owner：「数据会过期，机制不会；目录不要复制粘贴」）：
 *
 *   L1 yaml 种子（本脚本处理的一层）：catalog.source.yaml →
 ...
 *
 * 源文件（单一事实来源）：
 *   packages/host/src/providers/catalog.source.yaml
 *
...
const SOURCE = path.join(ROOT, "packages/host/src/providers/catalog.source.yaml");
