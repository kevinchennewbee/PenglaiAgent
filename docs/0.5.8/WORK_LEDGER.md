...
| P058-040 | Penglai settings locale and error boundary | Source/UI/native | IN_PROGRESS | settings no longer render caught exception/catalog text; Plugin Center activation/rollback now exposes a stable one-way diagnostic reference beside closed codes without leaking its private transaction ID; other typed references, authoritative outage deduplication, official DSH labels, and installed readback remain |
