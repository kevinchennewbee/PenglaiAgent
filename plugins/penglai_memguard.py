...
_RUNTIME_MEMORY_GOVERNOR = MemoryGovernor()
...
def _scan(content):
    ...
    decision = _RUNTIME_MEMORY_GOVERNOR.classify(content, context={"target": "memory_write"})
    if not decision.should_write and decision.reason in {
        "runtime_or_tool_noise",
        "contains_secret_shape",
    }:
        return f"运行时记忆治理:{decision.reason}"
...
def _atomic_write_bytes(fp, data):
    """原子落盘：同目录 temp 文件 + os.replace。还原是恢复路径，
    崩溃中途不能留半截记忆文件（记忆会注入未来提示词）。"""
    d = os.path.dirname(fp)
    os.makedirs(d, exist_ok=True)
    fd, tmp = tempfile.mkstemp(dir=d, prefix=".penglai_restore_")
    try:
...
def _enforce_memory(snap):
    ...
    for fp in current:
        ...
        if fp in snap:
            if now != snap[fp]:
                try:
                    _atomic_write_bytes(fp, snap[fp])
