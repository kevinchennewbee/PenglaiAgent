...
| Windows process tree | Electron supervises helper that can outlive child Node | Upstream has lower-level Windows helpers, not Penglai recovery policy | Fixed the one-sided helper wait, dual-lifetime handshake, stale facade state, duplicate supervisor risk, and restart port drift; port-loss/diagnostics remain | Reconcile helper selection after package closure | Child death detected, bounded restart, no orphan, coherent degraded state |
