...
test("voice source metadata is strict and enters only the model pre-step view", () => {
  ...
  assert.match(
    entered[0]?.content[0] && (entered[0].content[0] as { text: string }).text,
    /NOT USER-AUTHORED/,
  );
  assert.match(
    entered[0]?.content[0] && (entered[0].content[0] as { text: string }).text,
    /language=zh; emotion=HAPPY/,
  );
  assert.equal(
    (entered[0]?.content[1] as { text?: string } | undefined)?.text,
    "今天很开心",
  );
  assert.equal(entered[0]?.id, durable.id);
  assert.equal(entered[0]?.source, durable.source);
  assert.equal(isJsonValue(entered[0]), true);
  assert.deepEqual(Reflect.ownKeys(entered[0] ?? {}), [
    "id",
    "role",
    "content",
    "source",
  ...
  assert.equal(
    extractPenglaiSource({
      ...source,
      voice: { language: "zh", emotion: "USER_SUPPLIED" },
    }),
    undefined,
  );
...
test("bridge followup submits official DSH image blocks instead of media captions", async () => {
  ...
  assert.equal(
    sent.some(
      (row) => row.type === "image" && row.attachment?.attachmentId === "att-1",
    ),
    true,
  );
  assert.equal(
    sent.some(
      (row) =>
        row.type === "text" && String(row.text).includes("penglai-media"),
    ),
    false,
  );
});
...
test("bridge supplies the session-bound opaque office handle to the official DSH turn", async () => {
  ...
  const context = sent.find(
    (part) => part.type === "text" && part.text?.includes("office_handle="),
  );
  assert.match(
    context?.text ?? "",
    /office_handle=obj-0123456789abcdef01234567/,
  );
  assert.doesNotMatch(
    context?.text ?? "",
    /[/\\](Users|Volumes|home|tmp)[/\\]/,
  );
});
...
test("bridge submits official FileBlock plus office handle text and never a host path", async () => {
  ...
  assert.equal(
    sent.some(
      (row) =>
        row.type === "file" &&
        row.attachment?.attachmentId === file.attachmentId,
    ),
    true,
  );
  assert.equal(
    sent.some(
      (row) =>
        row.type === "text" && String(row.text).includes("office_handle="),
    ),
    true,
  );
  assert.equal(
    sent.some(
      (row) =>
        String(row.text ?? "").includes("/Users/") ||
        String(row.attachment?.name ?? "").includes("/"),
    ),
    false,
  );
});
...
test("bridge treats a durable DSH inbox message id as an idempotent replay", async () => {
  ...
  const bridge = new DshBridge({
    ...
    inspectSession: async (id) => (id === "s" ? { events } : undefined),
    listWorkspaces: () => [{ id: "w", title: "W", sessionIds: ["s"] }],
  });
  const input = {
    sessionId: "s",
  ...
  assert.deepEqual(await bridge.followup(input), {
    dshMessageId: "already-durable",
  });
  assert.deepEqual(await bridge.steer({ ...input, mode: "steer" }), {
    dshMessageId: "already-durable",
  });
  assert.deepEqual(calls, []);
});
...
test("session projection refreshes rename, deletion, Unicode, duplicates, and large owner lists", async () => {
  ...
  workspaceSessionIds = [
    "session-b",
    ...Array.from({ length: 150 }, (_, index) => `many-${index}`),
  ];
  sessionRows = [
  ...
  assert.equal(
    refreshed.some((session) => session.id === "session-a"),
    false,
  );
...
test("bridge forwards a new-session title only to the Session owner", async () => {
  ...
  assert.deepEqual(await bridge.createSession("workspace-1", "Penglai"), {
    id: "session-created",
  });
  assert.deepEqual(calls, [
    { workspaceIdentity: "workspace-1", title: "Penglai" },
  ]);
});
...
test("rc.2 adapter contains apiProxy session.create as the historical new-session seam", async () => {
  ...
  const host = hostFromRc2Cordis(
    {
      ...
      workspaceRegistry: {
        list: () => [{ id: "workspace-1", title: "W", sessionIds: [] }],
      },
      apiProxy: {
        sessions: {
          async create(request) {
            ...
            return {
              result: {
                ok: true as const,
                value: { sessionId: "official-session-1" },
              },
            };
          },
        },
  ...
  assert.deepEqual(await host.createSession?.("workspace-1"), {
    id: "official-session-1",
  });
...
test("rc.2 adapter contains apiProxy session.models/selectModel as historical IM seams", async () => {
  ...
  const host = hostFromRc2Cordis(
    {
      ...
      workspaceRegistry: {
        list: () => [
          { id: "workspace-1", title: "W", sessionIds: ["session-1"] },
        ],
      },
      apiProxy: {
        sessions: {
          async create() {
            return {
              result: { ok: true as const, value: { sessionId: "session-2" } },
            };
          },
          ...
          async selectModel(request) {
            calls.push(
              `select:${request.payload.sessionId}:${request.payload.provider}/${request.payload.model}`,
            );
            return {
              result: {
                ok: true as const,
  ...
  assert.deepEqual(
    await host.selectSessionModel?.("session-1", {
      provider: "deepseek",
  ...
  assert.deepEqual(calls, [
    "models:session-1",
    "select:session-1:deepseek/deepseek-reasoner",
  ]);
});
...
test("official session/event pair delivers assistant final to the IM route", () => {
  ...
  listeners.get("session/event")?.(
    { id: "s" },
    {
      ...
      data: {
        turn: 2,
        message: { content: [{ type: "text", text: "penglai-causal-ok" }] },
      },
    },
  );
  listeners.get("session/event")?.(
...
test("official durable session recovery closes claimed-turn/final/outbox without duplicate delivery", () => {
  ...
  const assistant = {
    ...
    data: {
      turn: 3,
      message: { content: [{ type: "text", text: "durable-final" }] },
    },
  };
  ...
  recoverOfficialTurnDelivery(spliced.plane, {
    ...
    events: [
      ...
      {
        type: "agent/inbox/spliced",
        data: { inserted: [{ id: "mid-2", source: spliced.source }] },
      },
      {
        type: "assistant/message",
        data: {
          turn: 4,
          message: { content: [{ type: "text", text: "from-snapshot" }] },
        },
      },
  ...
  assert.equal(
    spliced.store.pendingOutbox("r")[0]?.payloadText,
    "from-snapshot",
  );
});

test("host recovery reads official inspect events and does not duplicate outbox", async () => {
  const { store, plane, source } = recoveryPlane();
  const events = [
    {
      type: "agent/inbox/claimed",
      data: { turn: 5, message: { id: "mid", source } },
    },
    {
      type: "assistant/message",
      data: {
        turn: 5,
        message: { content: [{ type: "text", text: "host-final" }] },
      },
    },
  ...
  const host = {
    ...
    getAgent: (id: string) =>
      id === "s"
        ...
        : undefined,
    inspectSession: async (id: string) => (id === "s" ? { events } : undefined),
    listWorkspaces: () => [],
  ...
  const events = [
    ...
    },
    {
      ...
      data: {
        turn: 7,
        step: 1,
        message: { content: [{ type: "text", text: "cold-final" }] },
        stream: [],
      },
    },
...
test("followup classifies a proven missing official session as INVALID_INPUT", async () => {
  ...
  await assert.rejects(
    ...
    (err: unknown) =>
      err instanceof PenglaiError &&
      err.errorClass === "INVALID_INPUT" &&
      /session does not exist/.test(err.message),
  );
});
...
test("recoverOfficialTurnDelivery uses pinned Session.append/snapshotEvents envelopes", () => {
  ...
  session.append(
    "assistant/message",
    {
      ...
      message: {
        role: "assistant",
        content: [{ type: "text", text: "PM-COLD-COMPLETED" }],
      },
      stream: [],
    } as never,
    { surfaceOp: "append" },
    ...
    events: [
      {
        ...
        data: {
          target: "next-turn",
          start: 0,
          inserted: [{ id: "mid-pre", source: first.source }],
        },
      },
      ...
      },
      {
        ...
        data: {
          turn: 2,
          message: { content: [{ type: "text", text: "from-splice" }] },
        },
      },
