...
| --- | --- | --- | --- | --- |
| `packages/channel-feishu/src/index.ts` callback/media boundary | Active product | P0-02/P0-03 | Terminal rejection containment plus closed request/stream/validation/admission/transcription phase and cause diagnostics are implemented; arbitrary vendor detail is rejected from audit | Rejected work cannot escape; retry class and closed cause are durable; Owner-live permission/download evidence remains |
