...
test("R50-TRUTH-001 identity pins are 0.6.2", () => {
  ...
  assert.equal(checked.productVersion, "0.6.2");
  recordAssertion({
    acceptanceId: "R50-TRUTH-001",
    runnerId: "release-identity.identity",
    testId: "identity-pins-0.6.2",
    assertionId: "productVersion-is-0.6.2",
    status: "PASS",
...
test("R50-TRUTH-002 candidateKind trustTier generation and three exact release targets", () => {
  ...
  assert.equal(checked.dsh, "0.1.5-rc.2");
  recordAssertion({
...
test("R50-TRUTH-008 publication fields match the owner-authorized public target", () => {
  ...
  assert.equal(id.publication.tag, "v0.6.2");
  assert.equal(id.publication.release, "v0.6.2");
  assert.equal(id.publication.channel, "stable-v0.6.2");
  assert.throws(
    () =>
      assertReleaseIdentity({
