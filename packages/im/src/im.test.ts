...
async function runTestOnlyCausalRoute(
  ...
  if (accepted.kind !== "accepted") {
    throw new PenglaiError(
      "DSH_UNAVAILABLE",
      `causal inbound rejected: ${accepted.text}`,
    );
  }
  ...
  throw new PenglaiError(
    "DSH_UNAVAILABLE",
    "causal Message→Turn→route did not complete",
  );
}
...
test("im runtime wires single control plane", async () => {
  ...
  const { token } = rt.plane.createPairing({
    workspaceIdentity: "w",
    sessionId: "s",
    adapter: "mock",
  });
...
test("optional voice capabilities resolve dynamically across hot plug and unload", () => {
  ...
  const asr = {
    stageAudio: async () => undefined,
    transcribe: async () => undefined,
  } as never;
  const tts = {
    synthesize: async () => undefined,
    readOutput: async () => undefined,
    releaseOutput: async () => undefined,
  } as never;
...
test("IM composes local ASR metadata through the official DSH pre-step", async () => {
  ...
  const decision = await addVoiceContextAtOfficialPreStep(async () => ({
    kind: "enter",
    messages: [durable],
  }));
  assert.equal((decision as any).kind, "enter");
  assert.match(
    (decision as any).messages[0].content[0].text,
    /language=zh; emotion=SAD/,
  );
...
test("pure IM starts without ASR/TTS and has no ad-hoc HTTP", async () => {
  ...
  const held: {
    name?: string;
    host?: { getOverview: () => Promise<{ plugin: string }> };
  } = {};
  ...
  assert.deepEqual(listeners.get("agent/pre-step")?.options, {
    global: true,
    prepend: true,
  });
  const plainDecision = await listeners
    .get("agent/pre-step")
    ?.listener({}, async () => ({
      kind: "enter",
      messages: [
        {
          content: [{ type: "text", text: "plain IM" }],
          source: { kind: "user" },
        },
      ],
    }));
...
test("R50-ROUTE-001/002/009 binding is official live list plus CAS and vendor target", async () => {
  ...
  assert.deepEqual(listed.workspaces[0]?.sessions, [
    { id: "s1", title: "正式会话" },
  ]);
  assert.throws(
    () =>
  ...
  assert.throws(
    () => rt.plane.requireVendorTarget(binding.id),
    /vendor reply target/,
  );
  rt.store.close();
});
...
test("R50-FS-002 Feishu App ID persists and secret stays a credential ref", async () => {
  ...
  const configured = await host.configureFeishu({
    appId: "cli_probe",
...
test("R2I-CRED-006 production vault refuses MemoryVault fallback", async () => {
  ...
  await assert.rejects(
    () => vault.write("penglai-im/weixin/default/token", "secret"),
    PenglaiError,
  );
...
test("weixin QR connected starts receive so the scanner can talk immediately", async () => {
  ...
  const overview = await host.getOverview();
  assert.equal(
    overview.channels.find((c) => c.channel === "feishu")?.connection,
    "connected",
  );
  (host.feishu as unknown as { status: string }).status = "degraded";
  const degraded = await host.getOverview();
  assert.equal(
    degraded.channels.find((c) => c.channel === "feishu")?.connection,
    "degraded",
  );
  rt.store.close();
});
...
test("Weixin QR protocol failure returns and persists one redacted public cause", async () => {
  ...
  const overview = await host.getOverview();
  const failure = overview.channels.find(
    (row) => row.channel === "weixin",
  )?.error;
  assert.equal(failure?.code, "CHANNEL_PROTOCOL");
  assert.equal(
    failure?.referenceId,
    "failure" in result ? result.failure.referenceId : "",
  );
...
test("legacy IM failure storage migrates and retains only safe transport fields", () => {
  ...
  try {
    ...
    assert.equal(bots.getChannelFailure("weixin", "weixin-default"), undefined);
...
test("R2I-ROUTE packaged causal Message→Turn→route stays on original route", async () => {
  const agent = {
    ...
    followup(message: {
      ...
      rt.plane.onAssistantFinal({
        sessionId: "s1",
        turnId: "7",
        text: "token penglai-causal-ok",
      });
    },
    steer() {},
    cancel() {},
  ...
  const hostLike = {
    version: "0.1.2-alpha.1" as const,
    getAgent: () => agent,
    listWorkspaces: () => [{ id: "w", title: "W", sessionIds: ["s1"] }],
  };
  ...
  const proof = await runTestOnlyCausalRoute(host, {
    workspaceId: "w",
    sessionId: "s1",
  });
...
test("R2I-CRED-007 IM credential refs are official POSIX identifiers and vault rejects invalid refs", async () => {
  const { WEIXIN_TOKEN_REF, FEISHU_SECRET_REF, CHANNEL_CREDENTIAL_REFS } =
  ...
  for (const ref of Object.values(CHANNEL_CREDENTIAL_REFS))
    assert.match(ref, official);
  ...
  const credentials = {
    ...
    async describe(ref: string) {
      return { configured: store.has(ref), source: "yaml", writable: true };
    },
    async resolve(ref: string) {
      return store.has(ref)
        ? { value: store.get(ref) as string, source: "yaml" }
        : undefined;
    },
  ...
  await assert.rejects(
    () => vault.write("penglai-im/weixin/default/token", "x"),
    PenglaiError,
  );
  await assert.rejects(
    () => vault.write("penglai-im/feishu/default/app-secret", "x"),
    PenglaiError,
  );
});
...
test("R2I-CRED-008 legacy weixin-bot-token migrates to canonical ref and supervisor resumes receive", async () => {
  const {
    ...
    migrateWeixinTokenRef,
    LEGACY_WEIXIN_TOKEN_REF,
  } = await import("../../channel-weixin/src/index.js");
  const { WEIXIN_TOKEN_REF } = await import("./credentials-vault.js");
  assert.equal(WEIXIN_TOKEN_CREDENTIAL_REF, WEIXIN_TOKEN_REF);
  const store = new Map<string, string>([
    [LEGACY_WEIXIN_TOKEN_REF, "legacy-tok"],
  ]);
  const credentials = {
    async set(ref: string, value: string) {
      store.set(ref, value);
  ...
  assert.equal((await vault.describe(WEIXIN_TOKEN_REF)).configured, true);
  const empty = await migrateWeixinTokenRef(
    new CredentialsServiceVault({
      async set() {},
      async describe() {
...
test("Feishu owner is required for inbound and persists without appearing in overview", async () => {
  ...
  assert.equal(
    JSON.parse(rt.store.getAdapterConfig("feishu-default") ?? "{}").ownerOpenId,
    undefined,
  );
