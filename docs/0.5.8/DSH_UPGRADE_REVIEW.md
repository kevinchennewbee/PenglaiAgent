...
> Review refreshed: 2026-08-30. The Owner has fixed the source baseline and
> authorized a reproducible local tarball closure. The official DSH repository,
> fixed source, and upstream pack/clean-install implementation are authoritative;
> this document is a Penglai integration review, not an upstream specification.
...
1. bind the Owner-selected source tag to an exact commit and verify that source
   independently;
2. complete source-level package/API/slot inventories, characterization tests,
   and DSH-independent Penglai repairs on `0.5.8-preview`;
3. build both upstream release families and reconcile every local tarball and
   generated artifact with the fixed source before changing dependencies;
4. create a minimal packaged new-DSH Penglai compatibility skeleton;
5. migrate all removed host and client seams;
6. reproduce every 0.5.7 finding on that skeleton; and
7. fix only the remaining failures owned by Penglai.

This resolves the apparent “both/and” requirement: 0.5.8 both preserves and
stabilizes the 0.5.7 product functions, and adopts the fixed DSH baseline, without
...
| Candidate Git object | lightweight tag resolving directly to a commit |
| Candidate tree | `a712eec535b48badc4fefb4df5176a7002e4280b` |
| Deterministic `git archive` SHA-256 | `1fe7d2380d3e53eac2f6ee92ee5c81850ddc9b735b5910bae132cf1fc12b7211` |
| GitHub release state | prerelease, not draft; published 2026-08-27 |
| Uploaded GitHub assets | none; Penglai uses the fixed source and upstream release packer rather than waiting for npm |
...
| Clean upstream frozen-lock install | PASS on macOS arm64, Node `22.22.2`, pnpm `11.7.0` |
| Upstream full source build | PASS, including Host, Client, CLI, and Web frontend |
| Source package manifests | 254 under `packages/`; 25 added and 5 removed versus rc.2 |
...
alpha client implementation; P058-014 starts after the local closure passes.
...
packed package contracts still require reconciliation.
...
Memory audit. Alpha.1 local export reconciliation plus packaged restart,
...
The preview source now implements the cross-version startup seam before product
closure integration. It recognizes only the fixed-source alpha.1
...
does not prove that the source-built alpha package starts inside a native build.
...
source/corpus result, not proof for source-built package bytes, a real installed
...
exact source-built alpha.1 process.
...
- a source checkout or partial tarball set presented as a reproducible product
  closure; or
...
- exact local tarball versions, SHA-256 values, and identities for DSH plus all
  referenced packages;
- mutually consistent package dependencies and release-family membership;
...
- source/package provenance, upstream pack commands, and closure graph;
