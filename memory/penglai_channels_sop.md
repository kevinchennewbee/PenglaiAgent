...
> ⛔ **绝对不要在 code_run/工具里替用户跑 `penglai enable <渠道>`**。它是交互式的(要扫码、要等手机确认),
> 在没有终端的环境里会**卡死等输入**(用户实测踩过:在 TUI 里让你代跑 enable qq,整个卡住)。
> 你的正确动作:把 `penglai enable <渠道>` 这条命令**用文字发给用户**,让他在自己的终端运行。
> (enable 已加非 tty 保护会直接拒绝,但你根本就不该尝试。)
