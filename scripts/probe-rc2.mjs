...
writeFileSync(join(outDir, "rc2-contracts.json"), JSON.stringify(rec, null, 2));
writeFileSync(join(outDir, "rc1-contracts.json"), JSON.stringify(rec, null, 2));
console.log("probe-rc2", JSON.stringify({ dsh: baseline.dsh, lark: larkOk.version, overlay: baseline.overlay.sidebarWordmarkHasNameProp }));
