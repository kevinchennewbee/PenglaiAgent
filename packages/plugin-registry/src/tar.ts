...
function octal(block: Buffer, start: number, length: number): number {
  ...
  if (!text) return 0;
  if (!/^[0-7]+$/.test(text)) throw new PenglaiError("SECURITY_POLICY", "tar numeric field is not octal");
  const value = Number.parseInt(text, 8);
  if (!Number.isSafeInteger(value) || value < 0) throw new PenglaiError("SECURITY_POLICY", "tar numeric field overflow");
  return value;
}
...
export function inspectTarGz(bytes: Buffer): SafeTarEntry[] {
  ...
  while (offset + BLOCK <= tar.length) {
    ...
    if (bodyEnd > tar.length) throw new PenglaiError("SECURITY_POLICY", "truncated tar entry");
    ...
    if (type === "x") pendingPax = parsePax(body);
    else if (type === "g") globalPax = { ...globalPax, ...parsePax(body) };
    else if (type === "L") {
      const nul = body.indexOf(0);
      longPath = body.subarray(0, nul < 0 ? body.length : nul).toString("utf8");
    } else {
      const path = safeArchivePath(pendingPax.path ?? globalPax.path ?? longPath ?? headerPath);
      if (path) {
        if (seen.has(path)) throw new PenglaiError("SECURITY_POLICY", `duplicate archive path ${path}`);
        seen.add(path);
        if (type === "\0" || type === "0") entries.push({ path, kind: "file", mode, data: Buffer.from(body) });
        else if (type === "5") entries.push({ path, kind: "directory", mode, data: Buffer.alloc(0) });
        else throw new PenglaiError("SECURITY_POLICY", `archive entry type ${JSON.stringify(type)} is forbidden`);
      }
      pendingPax = {};
      longPath = undefined;
      if (entries.length > MAX_ENTRIES) throw new PenglaiError("SECURITY_POLICY", "archive entry limit exceeded");
    }
