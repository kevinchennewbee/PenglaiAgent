...
| Locale | Penglai client strings plus built overlay assumptions | Third-party locale registration and typed client locale | ASR composer raw `mic`/`stop` are replaced with localized accessible lifecycle labels; raw `read` and remaining plugin copy stay inventoried | Reconcile exact published locale package and complete remaining client strings | Fresh Chinese, English, runtime switch, accessible labels |
