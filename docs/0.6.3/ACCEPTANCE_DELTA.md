...
- Disable the upstream session log/upload behavior and free-form plugin manager;
  keep only Penglai's signed catalog installation path.
...
- Keep LibreOffice, Office/PDF, Budget, and Companion absent from the workspace,
  profile, catalog, runtime closure, installer, and product SBOM. Memory is the
  only required first-party feature plugin. The complete upstream cohort ledger
  is audit evidence, not a requirement to ship excluded packages.
...
The current authorization ends after merge to `main`; every native, installed,
upgrade, package, publication, and public-readback result remains `NOT_RUN`.
...
profile、catalog、运行闭包、安装包或产品 SBOM。记忆是唯一必装的第一方功能插件；
完整上游 cohort 清单只用于审计上游输入，不等于全部随包。
