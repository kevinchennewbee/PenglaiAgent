...
test("R50-UPD-008/009/010 verified installer and crash replay", () => {
  ...
  const path = join(root, "Penglai_0.5.6_macos_aarch64.dmg");
...
test("verified installer handoff refuses a symlink even when target bytes are signed", () => {
  ...
  const linked = join(root, "Penglai_0.5.6_macos_aarch64.dmg");
  const payload = Buffer.from("signed-installer");
...
test("R50-DIST: packaged identity is Penglai 0.5.6 and Windows NSIS stays current-user", async () => {
  const {
  ...
  assert.equal(facts.shortVersion, "0.5.6");
