...
test("official DSH ships Pi adapter and DeepSeek adapter at the pinned version", async () => {
  ...
  assert.equal(piManifest.version, "0.1.7-alpha.2");
  assert.equal(deepseekManifest.version, "0.1.7-alpha.2");
...
test("completeWelcome writes the fixed DSH welcomeNoticeVersion then advances welcome-v1", async () => {
  ...
  assert.deepEqual(ops, [
    { ns: "ui-settings-general", path: ["welcomeNoticeVersion"], value: DSH_WELCOME_NOTICE_VERSION },
  ]);
  assert.equal(DSH_WELCOME_NOTICE_VERSION, "2026-08-13.1");
  const npmCohort = JSON.parse(
    readFileSync(new URL("../../../docs/0.6.6/DSH_NPM_COHORT.json", import.meta.url), "utf8"),
  );
  assert.equal(DSH_WELCOME_NOTICE_VERSION, npmCohort.upstreamFacts.welcomeNotice.version);
  assert.equal(readWelcomeAckFromOfficialSettings([{ ns: "ui-settings-general", value: { welcomeNoticeVersion: DSH_WELCOME_NOTICE_VERSION } }]), true);
  assert.equal(readWelcomeAckFromOfficialSettings([{ ns: "ui-onboarding", value: { welcomeNoticeVersion: DSH_WELCOME_NOTICE_VERSION } }]), false);
