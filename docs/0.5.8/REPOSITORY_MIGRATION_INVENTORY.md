...
| `apps/desktop/src/supervisor.ts` and Windows helper | Active product | P0-01 | Dual child/owner lifetime wait and fail-closed helper handshake implemented; health/diagnostic work continues | Child exit/hang/port loss detected; bounded recovery; no orphan |
...
| packaged DSH process ownership | 5 source files and 5 test files | Penglai desktop/runtime policy, using platform-native ownership | One-sided Windows wait fixed in source; native child-exit, port-loss, and process-tree evidence remains |
