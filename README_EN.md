...
You can override the mirror with `PENGLAI_GH_PROXY=https://your-mirror/`. If you do not trust the default mirror, download the script first, then run it with your own `PENGLAI_GH_PROXY`.
...
- **2026-06-17** — v0.2.10: **Feishu real-workflow fixes** — Feishu no longer drops a second message while the previous task is still running; follow-up messages are queued and processed automatically after the active task finishes. Invalid Feishu button clicks no longer consume a pending human-intervention choice. When the model's final text is empty but files were generated, the task card now says the files are being sent instead of reporting a misleading truncation warning. Feishu logs redact API keys, tokens, and secrets. `[FILE:]` sending is no longer directory-restricted; it only blocks the `.py`, `.env`, `.key`, `.sh`, and `.pem` sensitive suffixes, so videos, images, PDFs, Office documents, and other real work artifacts can be sent from practical locations such as Downloads. Audio/video files are sent as Feishu file messages to avoid media type mismatch errors. All changes live in the Penglai layer; upstream GA code is unchanged.
- **2026-06-17** — v0.2.9 hotfix: **redaction and third-party compliance closeout** — Telegram examples in `mykey_template.py` are now obvious placeholders; desktop-pet skins now carry third-party license metadata and the Glube skin with no standard redistribution license was removed; CDP Bridge is documented as a high-permission developer tool; Docker now runs as a non-root user while keeping `/data` volume state and WeChat token paths working, and `.dockerignore` keeps local private runtime state out of build contexts; install scripts, Docker install, `penglai update`, and the PyPI bootstrap all support `PENGLAI_GH_PROXY`; Tauri now has a CSP instead of `null`; `SECURITY.md` and `THIRD_PARTY_NOTICES.md` were added.
- **2026-06-17** — v0.2.9: **proactive companion that is actually felt + image/voice/file-send hardening** — proactive companion now defaults to `present` mode: weather, voice-emotion follow-up, and morning/evening anchors must send one short message after passing deterministic gates instead of disappearing into `[SILENT]`; `penglai abilities` now shows the latest heartbeat, silence/failure reason, critic review status, and L1/L2 memory health; vision fixes OpenAI-compatible `/v1/v1` endpoint joins and repairs already-generated `vision_api.py`; Feishu `[FILE:]` output now gets batch-preflighted so safe files still send while bad paths collapse into one concise notice; local voice transcription now chunks long audio instead of loading the whole PCM into memory; audit logs avoid the source root by default, and `audit/` / `_internal/` are ignored for public release safety; `penglai update` recognizes both installed `origin/main` and this release checkout's `release/main`. New Penglai abilities still live in the Penglai layer, while this release also syncs GA upstream `continue`, Responses metadata, and runtime SOP fixes; upstream README/docs/demo assets are intentionally not imported.
- **2026-06-16** — v0.2.8: **Feishu human-intervention and macOS runtime closeout** — `ask_user` no longer shows only the dangling final sentence in Feishu: structured questions and choices render as clickable buttons, with numbered/exact-text replies as a fallback; Feishu now starts through the Penglai wrapper `penglai_feishu_app.py`, fixing `/review` routing while keeping upstream `frontends/fsapp.py` untouched and applying a compatibility patch for lark-oapi WebSocket CARD callbacks; the macOS Feishu frontend is now under **launchd supervision** too (legacy naked processes are cleaned up during migration, and crashes/kills reconnect automatically); the WeChat wrapper gains `/new` / `/restore` / `/continue` / `/btw` / `/review`; `fs_owner_open_id` is separated from the access allowlist so owner notifications no longer depend on an open allowlist; `penglai update` now adds low-speed/overall timeouts to direct GitHub fetches and falls back to gh-proxy instead of hanging; doctor now reports the real `.venv` Python and launchd state. Verified on Tencent Cloud and Mac mini, **zero GA-kernel diff**.
- **2026-06-16** — v0.2.7: **macOS field-hardening pass** (driven by an overnight live forensic + multi-track audit) — 💞 proactive companion / ⏰ reminders now run under **launchd on macOS** (auto-restart on crash/kill, start at boot — parity with Linux; the old build left them dead-on-kill and reminders silently never fired) + 👁️ **images actually get seen** (wired to your main multimodal model + an "IM image → look" rule, no more guessing "I have no vision" from the model name) + 🔒 **key-leak prevention** (the assistant no longer prints API keys into logs when checking config; covers every search-source key) + 🎙️ voice gives a readable error when ffmpeg is missing instead of failing silently + 🛡️ proactive warning + one-line tighten when the Feishu allowlist is empty + `penglai update` China-mirror fallback. All in the Penglai layer, **zero GA-kernel diff**, field-verified on both platforms
- **2026-06-15** — v0.2.6: **enable abilities without a restart** (after `penglai enable critic/intel/companion` writes your key/city, **the very next message uses it** — the long-running process hot-reloads mykey, no service restart needed) + **restart phones home** (when the agent restarts itself, it proactively sends "I'm back" to your Feishu/WeChat once it reconnects, instead of leaving you waiting in silence) + **setup wizard detects your old assistant** (spots Hermes/OpenClaw and offers to bring over your **memory/model/channels/persona** in one step, instead of configuring from scratch; migration preview field-verified against a real Hermes) — all in the Penglai layer, **zero GA-kernel diff**, field-verified
- **2026-06-15** — v0.2.5: **first wave of marketplace skills (8, all free to use)** — 📄 document read/write (Word/Excel/PPT/PDF, installs open-source libs on demand, offline & free) · 📰 WeChat article fetch (drop a link → body + images saved to Markdown) · 💰 voice expense log · 📦 parcel tracking · ✉️ email send/receive · 🗞️ daily news brief · ⏰ life reminders · 📊 market research — all native GA SOPs, security-scanned at install, triggers live in the L3 index so the every-turn memory never bloats + a **free-to-use rule** (every marketplace skill and any plugged-in MCP must be usable for free by ordinary users; nothing paywalled or enterprise-gated)
- **2026-06-14** — v0.2.4: **MCP client support** (point `mcp_servers` in `mykey` at any external MCP server and its tools plug straight in — pure stdlib, zero kernel diff, timeout-safe, args security-scanned) + **Agnes free onboarding** (the wizard ships Agnes AI's free all-modality tier — tool calls / image understanding / streaming — pick it and go) + **skill-marketplace memory refactor** (the trigger table moved to an on-demand L3 index; L1 keeps just one constant pointer — install as many skills as you like without bloating the every-turn ≤30-line core memory, honoring GA's first principle)
- **2026-06-14** — v0.2.3: synced upstream GenericAgent (13 commits, zero kernel diff, field-verified) — WeChat headless container login (QR goes straight to `docker logs`, no GUI needed) + TUI workspace project mode with `@`-file completion + a macOS accessibility ljqCtrl port
