...
> Review refreshed: 2026-08-29. The Owner has fixed the source baseline. Recheck
> npm and package facts immediately before dependency integration. The official
> DSH repository and published packages are authoritative; this document is a
> Penglai integration review, not an upstream specification.
...
3. when the official npm set appears, reconcile every published package and
...
source verification proves that the fixed commit installs and builds in an
isolated source checkout. It does not prove that npm tarballs exist, contain
the same generated bytes, form a complete closure, or can build Penglai. Exact
package availability, generated client artifacts, integrity values, and source
equivalence must be checked manually after formal package publication; no npm
monitor is part of this plan.
