...
export const WEIXIN_QR_PNG_PREFIX = "data:image/png;base64,";
const PNG_MAGIC = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);

export function isPngDataUrl(value: string): boolean {
  if (!value.startsWith(WEIXIN_QR_PNG_PREFIX)) return false;
  const buf = Buffer.from(value.slice(WEIXIN_QR_PNG_PREFIX.length), "base64");
...
export async function renderQrPngDataUrl(payload: string): Promise<string> {
  ...
  if (/^iVBOR[A-Za-z0-9+/]+=*$/.test(compact)) {
    const wrapped = WEIXIN_QR_PNG_PREFIX + compact;
...
export const renderWeixinQrImage = renderQrPngDataUrl;
