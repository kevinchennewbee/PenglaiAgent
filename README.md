...
0.3.0 preview 把飞书、桌面、终端和 IM wrapper 主链路收束到同一个 Runtime Hub：入口归一成 `InboundEvent`，同一 session 内 FIFO 排队，GA 继续作为极简执行内核，结果回到原入口。腾讯云已验证飞书长驻服务、真实 LLM 主链路、排队卡片、Runtime Hub 运行记录、安装预检、隐私审计和旧入口审计。

0.3.0 preview 不等于所有可选能力都默认开启：主动陪伴已按 opt-in 长驻服务验证，语音模型已在腾讯云验证 ready；批判脑异厂商复核、微信真收发和更多 IM 渠道仍按 `penglai abilities` / `penglai channels` 显示的状态逐项启用、诊断和验证。

近期更新重点：

- v0.3.0 preview：新增 Runtime Hub、`TaskRun`、统一队列、统一权限、Runtime Control API、桌面中枢面板、飞书 0.3.0 wrapper、终端 `runtime-chat`、安装预检、隐私/旧入口审计和可选语音能力诊断。
